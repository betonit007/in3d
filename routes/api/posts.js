...
//route POST - Comment on an existing posts
router.post('/comment/:id', [
    ...
    async (req, res) => {
      ...
      try {
        ...
        const user = await User.findById(req.user.id).select('-password'); // get 
        const post = await Post.findById(req.params.id); //find the original post to comment on
    ...
    router.delete('/comment/:id/:comment_id', auth, async(req, res) => {
        try {
            ...
            // Get remove index
            const removeIndex = post.comments.map(comment => comment.user.toString()).indexOf(req.user.id); // change object id to string and get index
