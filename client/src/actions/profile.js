...
export const getCurrentProfile = () => async dispatch => {
  ...
  try {
    const res = await axios.get('/api/profile/me'); //server will know which profile to load from token that is passed along
    dispatch({
      type: GET_PROFILE,
      payload: res.data
    })
...
export const addEducation = (formData, history) => async dispatch => {
  try {
    ...
    const res = await axios.put('api/profile/certifications', formData);
...
export const deleteEducation = id => async dispatch => {
  try {
    const res = await axios.delete(`/api/profile/certifications/${id}`);
...
export const deleteAccount = () => async dispatch => {
  ...
  if (window.confirm('Are you sure? This can NOT be undone')) {
    ...
    try {
      ...
      dispatch({
        type: PROFILE_ERROR,
        payload: { msg: error.response.statusText, status: error.repsonse.status }
      })
    }
  }
}